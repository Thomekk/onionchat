#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# ИСПРАВЛЕННАЯ ВЕРСИЯ – без мерцания, с мгновенным вводом, без баннера Flask
...
# Пути для Tor (ИСПРАВЛЕНО – убраны обратные слеши)
...
# ---------- Эндпоинты Flask (опущены для краткости, они должны быть здесь) ----------
# ... (полный список маршрутов из предыдущих версий, включая /api/poll, /api/send_message и т.д.)
...
class ChatClient:
    def __init__(self, stdscr):
        ...
        self.needs_redraw = True  # флаг для перерисовки
    ...
    def run(self):
        while self.running:
            ...
            if key != -1:
                ...
                # после обработки ввода перерисовка уже запланирована через needs_redraw
            else:
                # нет ввода – небольшой сон для снижения нагрузки
                curses.napms(20)  # 20 мс

# ------------------ Запуск Flask ------------------
def run_flask():
    init_db()
    flask_ready.set()
    # Запускаем с подавленными логами
