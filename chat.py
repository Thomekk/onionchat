#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# СТАБИЛЬНЫЙ ЧАТ – без трейсбеков, с надёжным вводом
...
def start_flask():
    ...
    # Передаём путь к файлу, где определена функция run_flask_server
    flask_process = subprocess.Popen(
        [sys.executable, '-c', 
         'import chat; chat.run_flask_server()'],
        stdout=subprocess.DEVNULL,
        stderr=subprocess.DEVNULL,
        start_new_session=True
    )
...
class ChatClient:
    def __init__(self, stdscr):
        ...
        curses.cbreak()              # немедленный ввод
        curses.noecho()              # не отображать вводимые символы автоматически
        self.stdscr.keypad(True)     # обрабатывать специальные клавиши
        self.stdscr.nodelay(True)    # неблокирующий режим
    ...
    def draw(self):
        ...
        self.stdscr.refresh()
        curses.curs_set(1)   # убеждаемся, что курсор видим
    ...
    def handle_input(self, key):
        if key == 10:  # Enter
            ...
            # При изменении размера просто запрашиваем перерисовку
            self.height, self.width = self.stdscr.getmaxyx()
    ...
    def run(self):
        while self.running:
            ...
            if key != -1:
                ...
                time.sleep(0.02)  # небольшой сон для снижения нагрузки
...
if __name__ == '__main__':
    ...
    try:
        ...
        stop_flask()
        stop_tor()
        print("> ", end='', flush=True)  # восстанавливаем приглашение