#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# СТАБИЛЬНЫЙ ЧАТ – исправлена блокировка ввода после повторного открытия клавиатуры
# + адаптивный заголовок и поле ввода прижимается к низу
...
class ChatClient:
    def __init__(self, stdscr):
        ...
        # Инициализация curses с правильной последовательностью
        self._init_curses()
    ...
    def _init_curses(self):
        """Инициализация curses с правильными настройками"""
        # Сначала базовые настройки
        curses.start_color()
        curses.init_pair(1, curses.COLOR_CYAN, curses.COLOR_BLACK)
        curses.init_pair(2, curses.COLOR_GREEN, curses.COLOR_BLACK)
        
        # Режимы ввода - важен порядок!
        curses.initscr()  # убедимся что ncurses инициализирован
        curses.noecho()   # не отображать ввод
        curses.cbreak()   # немедленный ввод без буферизации строки
        curses.raw()      # raw режим для корректной обработки всех клавиш
        
        # Настройка окна
        self.stdscr.keypad(True)  # включить обработку спецклавиш
        self.stdscr.timeout(20)   # неблокирующий ввод с таймаутом 20мс
        self.stdscr.idlok(True)   # разрешить вставку/удаление строк
        
        # Курсор
        try:
            curses.curs_set(1)    # показать курсор
        except curses.error:
            pass  # некоторые терминалы не поддерживают
        
        # Очистка буфера ввода при старте
        curses.flushinp()

    def _reinit_curses(self):
        """Полная переинициализация curses после изменения терминала"""
        try:
            # Очищаем буфер ввода от мусорных escape-последовательностей
            curses.flushinp()
            
            # Переустанавливаем все режимы
            curses.noecho()
            curses.cbreak()
            curses.raw()
            ...
            # Дополнительная очистка буфера
            curses.flushinp()
            
            # Небольшая задержка для стабилизации терминала
            time.sleep(0.05)
            ...
            # Если что-то пошло не так, пробуем восстановить базовые настройки
            try:
                curses.noecho()
                curses.cbreak()
                self.stdscr.keypad(True)
    ...
    def _update_terminal_size(self):
        """Обновляет размеры терминала и перенастраивает curses"""
        # Сначала обновляем размеры терминала
        # curses.resizeterm(0, 0) автоматически определит новые размеры
        try:
        ...
        # Переинициализируем curses для нового размера
        self._reinit_curses()
    ...
    def truncate_domain(self, domain, max_len):
        """Усекает домен до указанной длины с многоточием в середине"""
    ...
    def truncate_message(self, text, max_len):
        """Усекает сообщение до указанной длины с многоточием в конце"""
        if len(text) <= max_len:
            return text
        if max_len <= 3:
            return "..."[:max_len]
    ...
    def draw(self):
        """Отрисовка интерфейса - адаптивно под текущий размер экрана"""
        # ВСЕГДА обновляем размеры перед отрисовкой (важно для адаптивности)
        self.height, self.width = self.stdscr.getmaxyx()
        ...
        # --- Заголовок (строка 0) ---
        # Формируем заголовок динамически на основе ТЕКУЩЕЙ ширины
        base_header = "• Чат"
        header = base_header
        
        if self.friend_domain:
            # Рассчитываем доступное место для домена
            separator = " • "
            available_for_domain = self.width - len(base_header) - len(separator) - 1  # -1 для запаса
            
            if available_for_domain > 5:  # минимум 5 символов для отображения
                short_domain = self.truncate_domain(self.friend_domain, available_for_domain)
                header = base_header + separator + short_domain
            else:
                # Места мало - показываем только базовый заголовок
                header = base_header
        
        try:
            self.stdscr.addstr(0, 0, header[:self.width], curses.color_pair(1))
        except curses.error:
            pass

        # --- Разделительная линия (строка 1) ---
        try:
            self.stdscr.addstr(1, 0, "─" * min(self.width, self.width), curses.color_pair(1))
        except curses.error:
            pass

        # --- Область сообщений (строки 2 до height-2) ---
        # Зарезервировано: строка 0 - заголовок, строка 1 - разделитель, строка height-1 - ввод
        # Доступно для сообщений: строки с 2 по height-2 включительно
        chat_start_y = 2
        chat_end_y = self.height - 2  # последняя строка для сообщений (не включая поле ввода)
        chat_height = chat_end_y - chat_start_y
        
        if chat_height > 0:
            # Берём последние сообщения, которые помещаются в область чата
            ...
            for msg in display_messages:
                ...
                # Формат: "● HH:MM сообщение ..."
                base_line = f"{prefix}{time_str} "
                content = msg['content']
                
                # Добавляем индикатор доставки для исходящих
                if msg['direction'] == 'out' and not msg.get('delivered', True):
                    content += " ⏳"
                
                # Рассчитываем доступную ширину для содержимого
                available_width = self.width - len(base_line)
                
                if available_width > 0:
                    # Обрезаем содержимое если нужно
                    if len(content) > available_width:
                        content = self.truncate_message(content, available_width)
                    line = base_line + content
                else:
                    # Очень узкий экран - обрезаем всю строку
                    line = base_line[:self.width]
        ...
        # --- Поле ввода (ПОСЛЕДНЯЯ строка - прилипает к низу) ---
        # При открытии клавиатуры height уменьшается и поле ввода автоматически
        # перемещается в новую последнюю строку
        input_line = self.height - 1
        input_prompt = "> "
        input_content = self.input_buffer
        
        # Проверяем помещается ли всё содержимое
        max_input_len = self.width - len(input_prompt)
        if max_input_len > 0:
            # Если ввод слишком длинный, показываем только конец (скролл)
            if len(input_content) > max_input_len:
                # Показываем последние символы чтобы пользователь видел что печатает
                visible_input = input_content[-(max_input_len):]
            else:
                visible_input = input_content
            
            try:
                self.stdscr.addstr(input_line, 0, input_prompt + visible_input)
                self.stdscr.clrtoeol()  # очищаем остаток строки
                # Курсор в позицию ввода (учитывая скролл если есть)
                cursor_pos = len(input_prompt) + len(visible_input)
                if cursor_pos < self.width:
                    self.stdscr.move(input_line, cursor_pos)
                else:
                    self.stdscr.move(input_line, self.width - 1)
            except curses.error:
                pass
    ...
    def _is_real_escape(self):
        """
        Проверяет, является ли нажатие ESC одиночным или частью escape-последовательности.
        Возвращает True если это действительно ESC (пользователь нажал Escape).
        """
        # Сохраняем текущий таймаут
        old_timeout = self.stdscr.gettimeout()
        
        # Устанавливаем короткий таймаут для проверки
        self.stdscr.timeout(50)  # 50мс - достаточно для escape-последовательности
        
        next_char = self.stdscr.getch()
        
        # Восстанавливаем таймаут
        self.stdscr.timeout(old_timeout if old_timeout is not None else 20)
        
        # Если есть следующий символ - это часть escape-последовательности
        if next_char != -1:
            # Это escape-последовательность, возвращаем символы в буфер (игнорируем)
            # flushinp очистит остатки последовательности
            curses.flushinp()
            return False
        
        # Нет следующего символа - это одиночный ESC
        return True

    def handle_input(self, key):
        # -1 означает "нет ввода" - просто игнорируем
        if key == -1:
            return
        ...
        if key == 10:  # Enter
        ...
        elif key == 3:  # Ctrl+C - корректный выход
            self.running = False
            
        elif key == 27:  # ESC - требует специальной обработки
            # Проверяем, является ли это одиночным ESC или частью escape-последовательности
            if self._is_real_escape():
                # Это действительно ESC - выходим
                self.running = False
            # Иначе это была escape-последовательность от терминала - игнорируем
            
        elif key in (ord('q'), ord('Q')):  # q или Q для выхода
            self.running = False
            ...
            # Изменение размера терминала (открытие/закрытие экранной клавиатуры)
            # Обновляем размеры и переинициализируем curses
            self._update_terminal_size()
            
        elif key == curses.KEY_DL or key == curses.KEY_CLEAR:  # Clear line
            self.input_buffer = ""
            self.needs_redraw = True
            
        elif 32 <= key < 127:  # Печатные ASCII символы
            self.input_buffer += chr(key)
            self.needs_redraw = True
            
        # Игнорируем все остальные коды (включая escape-последовательности)
    ...
    def run(self):
        while self.running:
            ...
            try:
                ...
                # При ошибке curses переинициализируем
                self._reinit_curses()
                self.needs_redraw = True
